...
<<<<<<< HEAD
Code to get the data for players
```{r}
x_p <- lapply(1:12,function(j){
  y_j<-lapply(1:3, function(i) {
    p_p <- read_html(paste0("AFLW/html_pages/players/p",j,"_tab",i, ".html"))
    ptabs_p <- rvest::html_nodes(p_p, "table")
    players_ptab <- rvest::html_table(ptabs_p[[2]])
...
player_tab <- Reduce(merge, x_p)
colnames(player_tab)[1] <- "Club"
...
y <- lapply(1:3, function(i) {
  players_ptab <- read.csv(paste0("data_raw/players_tab", i ,".csv"), check.names = FALSE)
  colnames(players_ptab) <- paste0(colnames(players_ptab),"_", players_ptab[1, ])
  players_ptab <- players_ptab[-1, ]
  players_ptab
  players_ptab
...
>>>>>>> e18ea0802bd314811ef1d69b5bd0620f52b08f9d
```
